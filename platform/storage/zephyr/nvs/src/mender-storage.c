...
static mender_err_t
add_crc(char **data, size_t *data_len) {
...
static mender_err_t
check_crc(const char *data, const size_t data_len) {
    ...
    if (data_len > sizeof(uint32_t)) {
        ...
        /* Extract the CRC  */
        memcpy(&crc, data + data_len - sizeof(crc), sizeof(crc));
...
mender_storage_set_deployment_data(char *deployment_data) {
    ...
    if (MENDER_OK != add_crc(&deployment_data, &data_len)) {
        return MENDER_FAIL;
    }
...
#ifdef CONFIG_MENDER_STORAGE_DEPLOYMENT_DATA_CRC
    if (MENDER_OK != check_crc(*deployment_data, deployment_data_length)) {
        return MENDER_FAIL;
    }
#endif /* CONFIG_MENDER_STORAGE_DEPLOYMENT_DATA_CRC */
