...
mender_storage_get_authentication_keys(unsigned char **private_key, size_t *private_key_length, unsigned char **public_key, size_t *public_key_length) {
    ...
    if (MENDER_OK != ret) {
        if (MENDER_NOT_FOUND == ret) {
            mender_log_verbose("Private key not available");
        } else {
    ...
    if (MENDER_OK != ret) {
        if (MENDER_NOT_FOUND == ret) {
            mender_log_verbose("Public key not available");
        } else {
...
mender_storage_get_deployment_data(char **deployment_data) {
    ...
    if (MENDER_OK != ret) {
        if (MENDER_NOT_FOUND == ret) {
            mender_log_verbose("Deployment data not available");
        } else {
...
mender_storage_get_provides(mender_key_value_list_t **provides) {
    ...
    if (MENDER_OK != ret) {
        if (MENDER_NOT_FOUND == ret) {
            mender_log_verbose("Provides not available");
        } else {
