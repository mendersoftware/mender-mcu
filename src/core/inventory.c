...
static bool              persistent_pushed      = false;
...
mender_inventory_work_function(void) {
    ...
    if (!persistent_pushed && (NULL == persistent_inventory)) {
    ...
    if ((persistent_pushed || (NULL == persistent_inventory)) && (0 == n_callbacks - n_persistent_cbs)) {
        /* nothing to do */
        ret = MENDER_OK;
        goto END;
    }
    ...
    if (n_callbacks - n_persistent_cbs > 0) {
        for (uint8_t idx = 0; idx < n_callbacks; idx++) {
            if (callbacks[idx].persistent) {
                continue;
            }
